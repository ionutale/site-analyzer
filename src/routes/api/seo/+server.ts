...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const missingCanonical = await coll.countDocuments({ siteId, $or: [{ canonicalUrl: { $exists: false } }, { canonicalUrl: null }, { canonicalUrl: '' }] });
  const titleTooShort = await coll.countDocuments({ siteId, titleLength: { $gt: 0, $lt: minTitleLen } });
  ...
  const missingCanonicalPages = await coll
    .find({ siteId, $or: [{ canonicalUrl: { $exists: false } }, { canonicalUrl: null }, { canonicalUrl: '' }] }, { projection: { _id: 1, url: 1 } })
    .limit(20)
    .toArray();
  const shortTitlePages = await coll
    .find({ siteId, titleLength: { $gt: 0, $lt: minTitleLen } }, { projection: { _id: 1, url: 1, title: 1, titleLength: 1 } })
    .sort({ titleLength: 1 })
    .limit(20)
    .toArray();
  const longTitlePages = await coll
    .find({ siteId, titleLength: { $gt: maxTitleLen } }, { projection: { _id: 1, url: 1, title: 1, titleLength: 1 } })
    .sort({ titleLength: -1 })
    .limit(20)
    .toArray();
  ...
  const dupTitleAgg = await coll
    .aggregate<{
      ...
      { $group: { _id: { $toLower: { $trim: { input: '$title' } } }, count: { $sum: 1 }, urls: { $addToSet: '$url' }, title: { $first: '$title' } } },
      { $match: { count: { $gt: 1 } } },
      { $sort: { count: -1 } },
      { $limit: 20 }
    ])
  ...
  const dupMetaAgg = await coll
    .aggregate<{
      ...
      { $group: { _id: { $toLower: { $trim: { input: '$metaDescription' } } }, count: { $sum: 1 }, urls: { $addToSet: '$url' }, metaDescription: { $first: '$metaDescription' } } },
      { $match: { count: { $gt: 1 } } },
      { $sort: { count: -1 } },
      { $limit: 20 }
    ])
  ...
  return json({
    ...
    samples: {
      ...
      duplicateTitles: dupTitleAgg.map((d) => ({ key: d._id, count: d.count, urls: d.urls.slice(0, 5), title: d.title })),
      duplicateMeta: dupMetaAgg.map((d) => ({ key: d._id, count: d.count, urls: d.urls.slice(0, 5), metaDescription: d.metaDescription }))
    }
  });
};
